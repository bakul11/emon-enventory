...
const CategoryCart = ({ product, index }) => {
    const { category, photo, _id } = product;

    const handleRemoveProduct = async (id) => {
        const confirmRemove = window.confirm('Do you want delete this category?');
        if (confirmRemove) {
            fetch(`/api/category/removeCategory/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(result => {
            ...
            <td className='border-blue-100 border-b-[1px] p-2'>
                <div className="flex items-center gap-3 text-xl">
                    <div className="pd_update">
                        <Link href={`/home/category/update/${_id}`} >
