...
import CategoryCart from './(site)/CategoryCart';
...
const page = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const { data } = await axios.get(`/api/category/getCategoryUserBase/${userId}`)
                setProducts(data)
            } catch (error) {
                    ...
                    <div className="product">
                        {
                            products?.length === 0 ?
                                <div className="product-empty grid place-items-center my-12">
                                    <h2 className='text-slate-800 text-[19px] font-semibold'>Product Category is Empty</h2>
                                    <p className='text-gray-500 text-[15px] mb-2'>Please add category</p>
                                    <Link href='/home/category/add-category' className='inline-block ease-in-out transition-all duration-[0.5s] hover:bg-rose-500 bg-orange-400 p-3 text-[14px] rounded-md  capitalize text-white font-medium'>
                                ...
                                <div className='show-product '>
                                    <div className="flex items-center justify-between flex-wrap gap-5">
                                        <div className="product-title">
                                            <h2 className='text-slate-800 text-[19px] font-semibold'>Product Category list</h2>
                                        ...
                                        <table className=' w-full'>
                                            <thead>
                                                <tr className='text-[14px] text-left text-slate-700 font-[100] capitalize'>
                                                    ...
                                                    <th className=' border-blue-100 border-b-[1px] p-2'>category Name</th>
                                            ...
                                            <tbody>
                                                {
                                                    products?.filter((item) => item.category.toLowerCase().includes(search)).map((product, index) => <CategoryCart product={product} index={index} />)
