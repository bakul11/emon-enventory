...
const page = ({ params }) => {
    ...
    const categoryData = brand.toLocaleLowerCase();
        ...
        await fetch(`https://api.imgbb.com/1/upload?key=${process.env.NEXT_PUBLIC_IMAGE_SCRECT_KEY}`, {
            ...
            .then(data => {
                if (data?.success) {
                    ...
                    setLoadding(true);
                    fetch(`/api/brand/updateBrand/${params.id}`, {
                        method: 'PUT',
                        headers: {
    ...
    return (
        <div className='add-product'>
            <div className="product-title mb-8">
                <h2 className='text-slate-800 text-[19px] font-semibold'>Brand Edit</h2>
                <p className='text-gray-500 text-[14px]'>Update your Brand</p>
            </div>
            {/* add prouct form  */}
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-item">
                    <label htmlFor="ee" className='text-slate-500 my-1 font-medium text-[14px]'>Brand Name</label>
                    <input input id='ee' value={brand} className='bg-white p-2 my-2 text-[14px] outline-none w-full ring-1 ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-md ' placeholder='Name of brand' onChange={(e) => setBrand(e.target.value)} required={true} />
                </div>
                <div className="form-item my-5">
                    <label htmlFor="pppp" className='text-slate-500 my-1 font-medium text-[14px]'>Brand Image</label>
                    {
                        review?.length ?
                            <>
                ...
                <div className="form-submit my-12 bg-white">
                    {
                        loadding ?
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <Link href='/home/brands' className={`bg-rose-400 text-white rounded-md p-3 text-[14px] capitalize font-medium ${loadding ? 'hidden' : 'block'}`}>
                                    <div className="flex items-center gap-1">
                                        <MdClose />
                                        cancle
                                    </div>
