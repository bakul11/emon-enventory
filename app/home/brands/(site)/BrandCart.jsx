...
const BrandCart = ({ product, index }) => {
    const { brand, photo, _id } = product;

    const handleRemoveProduct = async (id) => {
        const confirmRemove = window.confirm('Do you want delete this Brand?');
        if (confirmRemove) {
            fetch(`/api/brand/removeBrand/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(result => {
            ...
            <td className='border-blue-100 border-b-[1px] p-2'>
                <div className="flex items-center gap-3 text-xl">
                    <div className="pd_update">
                        <Link href={`/home/brands/update/${_id}`} >
