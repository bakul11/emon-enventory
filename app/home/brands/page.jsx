...
import BrandCart from './(site)/BrandCart';
...
const page = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const { data } = await axios.get(`/api/brand/getBrandUserBase/${userId}`)
                setProducts(data)
            } catch (error) {
                    ...
                    <div className="product">
                        {
                            products?.length === 0 ?
                                <div className="product-empty grid place-items-center my-12">
                                    <h2 className='text-slate-800 text-[19px] font-semibold'>Product Brand is Empty</h2>
                                    <p className='text-gray-500 text-[15px] mb-2'>Please add Brand</p>
                                    <Link href='/home/brands/add-brand' className='inline-block ease-in-out transition-all duration-[0.5s] hover:bg-rose-500 bg-orange-400 p-3 text-[14px] rounded-md  capitalize text-white font-medium'>
                                ...
                                <div className='show-product '>
                                    <div className="flex items-center justify-between flex-wrap gap-5">
                                        <div className="product-title">
                                            <h2 className='text-slate-800 text-[19px] font-semibold'>Brand List</h2>
                                            <p className='text-gray-500 text-[15px]'>Manage your Brand</p>
                                        </div>
                                        ...
                                        <table className=' w-full'>
                                            <thead>
                                                <tr className='text-[14px] text-left text-slate-700 font-[100] capitalize'>
                                                    ...
                                                    <th className=' border-blue-100 border-b-[1px] p-2'>Brand Name</th>
                                            ...
                                            <tbody>
                                                {
                                                    products?.filter((item) => item.brand.toLowerCase().includes(search)).map((product, index) => <BrandCart product={product} index={index} />)
