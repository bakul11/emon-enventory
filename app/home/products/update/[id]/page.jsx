        ...
        await fetch(`https://api.imgbb.com/1/upload?key=${process.env.NEXT_PUBLIC_IMAGE_SCRECT_KEY}`, {
            ...
            .then(data => {
                if (data?.success) {
                    ...
                    setLoadding(true);
                    fetch(`/api/product/updateProduct/${params.id}`, {
                        method: 'PUT',
                        headers: {
                ...
                user?.email ?
                    <div className='add-product'>
                        <div className="product-title mb-8">
                            <h2 className='text-slate-800 text-[19px] font-semibold'>Product Edit</h2>
                            <p className='text-gray-500 text-[14px]'>Update your product</p>
                        </div>
                        {/* add prouct form  */}
