...
import AddSupplierModal from './(site)/AddSupplierModal';
import SupplierPayBtn from './(site)/SupplierPayBtn';
                    ...
                    <div className='add-product'>
                        <div className="flex items-center justify-between flex-wrap gap-5 my-12">
                            <div className="product-title">
                                <h2 className='text-slate-800 text-[19px] font-semibold'>Product Purchases</h2>
                                <p className='text-gray-500 text-[14px]'>Purchases latest Product</p>
                            </div>
                                ...
                                <div className="bg-white p-2 my-2 text-[14px] outline-none w-full ring-1 ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-md">
                                    <div className="bg-white flex items-center gap-2 justify-between rounded-md cursor-pointer" onClick={handleOpen}>
                                        <span className='capitalize '>
                                            {
                                                selectSupplierName ?
                                                    selectSupplierName?.length > 80 ?
                                                        selectSupplierName.substring(0, 100) + '...'
                                                        :
                                                        `${selectSupplierName} - 0${nbr}` :
                                                    'Select Supplier'
                                            }
                                        </span>
                                        <FaAngleDown />
                                        ...
                                        <div className="menu">
                                            {
                                                loadding ? <h3 className='text-slate-700 py-2'>Loading please wait...</h3>
                                                    ...
                                                    <>
                                                        {
                                                            supplier?.filter(pd => pd.userName.toLowerCase().includes(search)).map((pd, index) => {
                                                                const { userName, mobile, _id, email, address } = pd;
                                                                ...
                                                                return (
                                                                    <li
                                                                        ...
                                                                        onClick={() => {
                                                                            setSelectSupplierName(userName)
                                                                            setNbr(mobile)
                                                                            setCustomerId(_id)
                                                                            setEmail(email)
                                                                            setAddress(address)
                        ...
                        <SupplierPayBtn totalPay={getTotal().totalPrice} supplierData={supplierData} />
                    </div>
                    :
                    <LoaddingAnimation />
            }
