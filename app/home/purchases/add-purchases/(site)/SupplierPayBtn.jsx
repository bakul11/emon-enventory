...
const SupplierPayBtn = ({ totalPay, supplierData }) => {
    const { userName, userId, mobile, id, address, email } = supplierData;
    ...
    const handleSubmitPurchages = () => {
        ...
        fetch('/api/common/purchasesFromSupplier', {
            ...
            .then(result => {
                if (result.success) {
                    onCloseModal()
                    localStorage.setItem('supplier-invice', JSON.stringify(supplierStoreData))
                    localStorage.setItem('supplier-cart', JSON.stringify([...cart]))
                    router.push('/home/purchases/supplier-invoice')
